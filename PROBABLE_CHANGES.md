...
おっしゃる通り、LOCALフォルダのログは拡張子 .txt または .log と思われます。  
エラーが発生している具体的なログ内容を確認する必要があります。  
`%LOCALAPPDATA%\YukkuriMovieMaker\` 内にある `*.txt` または `*.log` ファイルをチャットに追加していただけますか？  
それらの内容を元に原因を特定し、必要な修正を提案します。
