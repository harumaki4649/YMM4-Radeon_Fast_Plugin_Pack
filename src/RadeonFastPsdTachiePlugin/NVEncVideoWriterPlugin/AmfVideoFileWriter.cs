...
internal sealed class AmfVideoFileWriter : IVideoFileWriter2, IDisposable
{
    ...
    private ID3D11Texture2D CopyTextureForQueue(ID3D11Texture2D source)
    {
        ...
        desc.MipLevels = Math.Max(1, desc.MipLevels);
        desc.ArraySize = Math.Max(1, desc.ArraySize);
        desc.Usage = ResourceUsage.Default;
        desc.CPUAccessFlags = CpuAccessFlags.None;
        desc.MiscFlags = ResourceOptionFlags.None;

        var copy = device.CreateTexture2D(desc, null);
        if (copy is null)
        {
            throw new InvalidOperationException("D3D11 キュー用テクスチャを作成できませんでした。");
        }
        ...
        {
            copy.Dispose();
            throw new InvalidOperationException("D3D11 ImmediateContext を取得できませんでした。");
        }
    ...
    private void ProcessVideoQueue()
    {
        try
        {
            ...
            {
                using var texture = item.Texture;
                ...
                lock (_encodeLock)
                {
                    ...
                    result = AmfNativeMethods.AmfEncode(_encoderHandle, texture.NativePointer);
                }
