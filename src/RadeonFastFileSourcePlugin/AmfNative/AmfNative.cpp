...
namespace
{
    ...
    bool DrainAmfOutput(EncoderState* state, bool waitForEof)
    {
        ...
        {
            state->profileDrainMs += QpcElapsedMs(profileStart);
            return value;
        };
    ...
    bool EnsureInputTexture(EncoderState* state, ID3D11Texture2D* source)
    {
        ...
        // Make the queued copy visible to AMF without blocking the CPU until the
        // GPU completes it. AMF and the copy use the same D3D11 device, so command
        // ordering is retained while avoiding a full GPU/CPU synchronization on
        // every frame.
    ...
    bool EncodeTexture(EncoderState* state, ID3D11Texture2D* texture)
    {
        ...
        state->profileEnsureTextureMs += QpcElapsedMs(ensureStart);

        amf::AMFSurfacePtr surface;
        const auto surfaceStart = NowQpc();
        auto result = state->context->CreateSurfaceFromDX11Native(state->inputTexture, &surface, nullptr);
        state->profileCreateSurfaceMs += QpcElapsedMs(surfaceStart);
        ...
        for (;;)
        {
            result = state->encoder->SubmitInput(encodeSurface);
            if (result == AMF_INPUT_FULL)
            {
        ...
        state->profileNativeMs += QpcElapsedMs(nativeStart);
        ++state->profileFrames;
